...
error <- no_glasses - mean(no_glasses)
hist(error)
car::densityPlot(error)
...
fit_lm <- lm(cumul_diff~log(abs(proprio_diff+.0001))) # compute absolute value of proprio_diff and add .001 to every value.
